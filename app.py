...
with col_up:
    ...
    arquivo = st.file_uploader("Arquivo Maxicon", type=["xlsx", "xls", "csv"],
                               label_visibility="collapsed")
with col_txt:
    st.subheader("Ou cole os dados")
    texto = st.text_area("Dados tabulados", height=120,
                         label_visibility="collapsed",
...
if df_raw is not None:
    ...
    with fc1:
        fil_granja = st.selectbox("Granja", granjas)
    with fc2:
        fil_raca = st.selectbox("Linhagem", racas)
    with fc3:
        fil_ano = st.selectbox("Ano", anos)

    res = df_res.copy()
    proj = df_proj.copy()
    if fil_granja != "Todas":
        res = res[res["granja"] == fil_granja]
        proj = proj[proj["granja"] == fil_granja]
    if fil_raca != "Todas":
        res = res[res["raca"] == fil_raca]
        proj = proj[proj["raca"] == fil_raca]
    if fil_ano != "Todos":
        proj = proj[proj["ano"] == int(fil_ano)]
    ...
    cores_rgba = {
        "COBB": "rgba(24,95,165,0.6)",
        "ROSS": "rgba(186,117,23,0.6)",
        "HUBB": "rgba(59,109,17,0.6)",
    }
    ...
    with c_top:
        ...
        fig3 = go.Figure(go.Bar(
            x=top10["total_ovos"].round(),
            y=top10["lote"],
            orientation="h",
            marker_color=[cores_rgba.get(r, "rgba(128,128,128,0.6)") for r in top10["raca"]],
        ))
        fig3.update_layout(
